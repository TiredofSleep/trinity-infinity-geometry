...
**Manuscript title:** *Joint Closure, Per-Coordinate Fuse Data, and a Closed-Form Algebraic Attractor of Two Commutative Binary Operations on $\mathbb{Z}/10\mathbb{Z}$*
...
We study a specific pair of commutative binary operations $T, B$ on $\mathbb{Z}/10\mathbb{Z}$, given explicitly by their Cayley tables and arising as the canonical $N=10$ instance of an operator-substrate composition family. Three exact structural results, all verified at machine precision or symbolically by deterministic Python scripts, are established.
...
The closest published precedent is **Drápal & Wanless (2021), *J. Combin. Theory Ser. A* **184**, 105510**, on maximally non-associative quasigroups — same domain (small finite commutative non-associative structures), opposite extremum (Drápal-Wanless: maximally non-associative; the present pair: structurally constrained with rational-and-algebraic invariants producing the closed-form attractor and Galois quartic).
...
- `4core_verification.py` — chain enumeration + per-coordinate fuse polynomials + Galois structure + α-sweep (6/6 PASS)
- `verification/04_bridge_attractor.py` — global attraction of $F_\alpha$ on the simplex
- `verification/06_attractor_closed_form.py` — symbolic derivation of $h/\beta = 1 + \sqrt{3}$
- `verification/07_full_closed_form.py` — quartic + Galois + LMFDB cross-check
- `verification/alpha_pslq_sweep.py` — Stern–Brocot $\alpha$-uniqueness scan

All scripts are archived in the shared Zenodo deposit at DOI 10.5281/zenodo.18852047.
