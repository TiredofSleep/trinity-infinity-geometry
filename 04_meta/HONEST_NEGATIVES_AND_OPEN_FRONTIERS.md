...
D184 maps substrate strands to *hydrogenic nodeless orbitals*. The extension to multi-electron atoms (where electron-electron interactions matter and the orbital concept becomes approximate) is **SPECULATIVE**. Whether the substrate-prime structure illuminates the periodic table beyond hydrogen is open.

### 3.2 The triple coincidence at d=3 is the only one

D185 shows Z/2310 has 32 divisors = Cl(0, 10) spinor dim = Pauli capacity at n = 4. Whether this triple coincidence occurs at other depths or only at d = 3 is **SPECULATIVE**. `clifford_substrate_shell.py` reports d = 1 also has a coincidence (Z/30 = Cl(0, 6) = n = 2 shell, all at 8) — convergence at *odd* depths. The pattern at d = 5, 7 etc. has not been audited.
