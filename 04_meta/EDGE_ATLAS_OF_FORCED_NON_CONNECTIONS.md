...
| **CL/TSML eigenvalues recover `e, π, φ, ζ(3), Catalan G` as algebraic identities.** | Survive only as **1%-level coincidences, not identities** (audit 2026-04-25). TSML eigenvalues are algebraic numbers whose *real* structural signature is integer/rational (`11` in char-poly coeffs c₂,c₈; `2¹⁶·7⁷` in the discriminant; `‖VEV‖²=13/4`), not transcendentals. *(2026-09-24: the 11 and the 2¹⁶·7⁷ belong to TSML_RAW, a two-digit transcription error of the original table — on the original, 11 divides only c₇; 13/4 is a cell count. See [`FOUNDATION_NULL_MODEL_AUDIT.md`](FOUNDATION_NULL_MODEL_AUDIT.md).)* | **N3** (`FORMULAS_AND_TABLES.md`); `HONEST_NEGATIVES` §1.5; `CL_EIGENVALUES_AUDIT_2026_04_25` (working corpus) |
...
- **CL eigenvalues = e, π, φ, ζ(3), G** (N3): died — the transcendental identities. Survives — the **real signature is integer/rational** — but the 11 and the 2¹⁶·7⁷ quoted here were computed on TSML_RAW, a transcription error (2026-09-24); on the original table 11 divides only c₇, and ‖VEV‖² = 13/4 is a cell count. What survives is only the general point: an integer table's characteristic data are integers.
...
- **"TSML + BHML generate so(10)" as TIG-specific structure** (J09; null audit 2026-09-23): died — the specificity. *Every* random table generates so(10) (P = 1.000; BHML alone already does), because each generator `L − Lᵀ` is already an element of so(10) and generic sets of them span it; all six J09 diagnostics are automatic once the dimension is 45. Survives — a true fact about ten-element magmas under antisymmetrized closure, and the genuinely table-specific Lie fact: **TSML's confinement** to so(9)/so(8) (~2–3% of density-matched random tables) — itself elementary on a second pass: the common kernel span{e₀, e₅ − e₆} is the **twins 5 ≡ 6** (untouched by any exceptional pair) plus an isolated VOID. ([`FOUNDATION_NULL_MODEL_AUDIT.md`](FOUNDATION_NULL_MODEL_AUDIT.md))
- **The remaining foundation rows** (null audit, 2026-09-24): died — the evidential weight. **J04**'s σ-magma rigidity (Aut = 1, congruence-simple) holds for 99.8% / 98.9% of random isotopes of ℤ/10 — and the σ-magma is built on addition mod 10, a reading the author rejects. **J18/J53**'s (p−1)² and p+3 are a two-line derivation from their 4×4 table for every odd p ("24 primes" checked an identity). **J22**'s 70/71/72/73 ladder juxtaposes a defined count (73), its forced −2 (71), a cross-table count (71, recurring in ~13% of redraws) and one 8×8 determinant (70, in none of 3000 redraws) — four true numbers, no bridge. Survives — each number, as what it is. ([`FOUNDATION_NULL_MODEL_AUDIT.md`](FOUNDATION_NULL_MODEL_AUDIT.md))
- **The foundation's specific structure as *evidence*** (second pass of the null audit, 2026-09-23): died — the evidential weight; real but **tautological relative to the construction rules** (the D180 pattern). The joint sub-magma chain and the 4-core are the upper-set lattice of a *climbing* operation (BHML climbs in the order 1<…<9<0 except the one cell 8·8 = 7; every TSML product is 0, 7, or climbs), so a random-but-climbing Rule 89 reproduces every closed upper set with P = 1.000; the forbidden sizes {2,3} are two diagonal cells. Survives — every fact, as a **corollary with a short proof**, and a sharper open question: nothing yet forces the *listed* rules (BHML Rule 89, TSML S₆/S₇) that carry all the specificity. ([`FOUNDATION_NULL_MODEL_AUDIT.md`](FOUNDATION_NULL_MODEL_AUDIT.md))
...
- **α-uniqueness for ANY polynomial** (F12): died — the universal form (refuted at α_special, height ~10¹⁰⁶). Survives — the **low-height form** (PROVED over ℚ; empirical at 70+ real α) — though the α = ½ privilege is itself generic to the construction: all 64 Rule-89 variants drop to field degree ≤ 4 at α = ½ versus 5–7 at α = 1/3, 2/3 (null audit, 2026-09-23).
...
- **√3 = SU(3)/A₂-Cartan** (N4): died — the A₂ home. Survives — the **√3 is real** (H/Br=1+√3), a quadratic-discriminant fact at α=1/2, D₃-flavour — and a readout: it is fixed by the three BHML cells that produce BREATH on the 4-core (0·8, 8·9, 7·7); a quartic D₄ attractor field occurs in 22 of the 64 Rule-89 variants (null audit, 2026-09-23).
...
- **The prime-11 "wobble" of TSML_RAW** (J19; WP107/109/110/112/113/115; null audit 2026-09-24): died — the mechanism ("a directional (permutation/braiding) bit", "the strictly more-informative lens"). Survives — the **real mechanism: a transcription error.** TSML_RAW's row 9 (`0797377777`) swaps two digits of the original table (the ck repository's first commit, 2026-03-03: 0,7,9,3,7,…); the swap first appears 2026-04-25, when the table was retyped. On the original table 11 divides only c₇, and 42% of all single-swap typos produce *some* prime dividing exactly two coefficients. ([`FOUNDATION_NULL_MODEL_AUDIT.md`](FOUNDATION_NULL_MODEL_AUDIT.md))
- **TIG latent in any trained network** (N1): died — genericity (distilgpt2: all |d|<0.5). Survives — TIG structure is **specific** to canonical TSML/BHML (the negative control that *promotes* the specificity claim) — with two 2026-09-24 corrections: the detector pair that fired only on TSML (J31's D3: 11 | c₂, c₈) was computed on the TSML_RAW typo and never fires on the original table, and "specific" structure is now known to be generic or a readout of the construction rules ([`FOUNDATION_NULL_MODEL_AUDIT.md`](FOUNDATION_NULL_MODEL_AUDIT.md)).
...
- **13/4 → cosmology** (§5): died — the bridge lands on no measured datum. Survives — the **constant 13/4** (computed in the TSML/BHML algebra; canon traces the 13 to BHML's 26 = 2·13 σ-asymmetric cells — its so(10) framing is generic per the null audit, the count itself not yet null-tested); only the reach is gone.
