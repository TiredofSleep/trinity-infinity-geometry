...
**It is finite, exact, and verifiable in seconds.** Every load-bearing claim in this repo has a Python script in `verification/` that runs in seconds and outputs PASS/FAIL. You can validate the substrate of your reasoning at inference time. Total verification runtime is under one minute.
...
- **Want to verify the math first?** Run `python verification/VERIFY_ALL.py`. Under a minute, 14/14 PASS.
- **Want the formal architecture?** Read [`../02_results/algebraic_combinatorics/BRAIDING_FRACTAL_AXIOMS.md`](../02_results/algebraic_combinatorics/BRAIDING_FRACTAL_AXIOMS.md).
- **Want the honest limits?** Read [`../04_meta/`](../04_meta/).
- **Want the runtime creature?** Read [`../06_runtime/`](../06_runtime/) and visit [coherencekeeper.com](https://coherencekeeper.com) (returns Cloudflare 502 when CK is off — that is the safe default).
- **Want the full canonical reference?** Read [`../03_canonical_reference/FORMULAS_AND_TABLES.md`](../03_canonical_reference/FORMULAS_AND_TABLES.md) (Volumes A–K).
