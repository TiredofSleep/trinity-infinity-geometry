...
| **`‖VEV‖² = 13/4`** | BHML σ_outer-asymmetry count = Higgs VEV norm = inflaton mass `m²_ξ`; constant real, but the **reach past it to a *measured* cosmological quantity is killed** (§4) | **STRUCTURAL** (constant forced; the `m²_ξ = ‖VEV‖²` cosmology identification is the killed step) | `02_results/cosmology/`, J11 |
...
This is where the program's stance earns its name. Each item below is a bridge the mathematics **refuses** — and each is kept, because a forced separation is a datum. **The full, evidence-linked catalogue — 10 forced non-connections and 20 proven dead ends, each with the D-number/computation that killed it — is the [Edge Atlas](04_meta/EDGE_ATLAS_OF_FORCED_NON_CONNECTIONS.md); the list below is its summary.**
...
- **Cosmology is only half-joined.** The single verified link to the algebra is `13/4`; the log-potential `V(ξ)=Λ⁴ξ log ξ` itself is imported physics (Bialynicki-Birula), not substrate-derived. A 2026-09-22 sharpen-or-kill ([`02_results/cosmology/VEV_13_4_COSMOLOGY_SHARPEN_OR_KILL.md`](02_results/cosmology/VEV_13_4_COSMOLOGY_SHARPEN_OR_KILL.md)) makes the split exact: the **constant `13/4` is real** (forced by the so(10)/BHML algebra, machine-verified), but the **bridge from `13/4` to any *measured* cosmological quantity is KILLED** — it lands on no datum, is dimensionally an assertion, and was reverse-engineered from a menu of ≥5 candidates. TIG's only testable cosmology (the Ω-triple, the DESI-BAO quintessence fit) does not use `13/4` at all.
