...
permits. (Beyond 4 the rules for realizing an integer diverge — equidistance runs out of
room, and the pentagon at *n* = 5 is the first regular polygon whose symmetry cannot tile
the plane, by the crystallographic restriction theorem [Barlow 1894]. Which rule realizes
which integer beyond 4 is set out in [`THE_INTEGERS.md`](THE_INTEGERS.md); it is not needed
for the main result here.)
...
tetrahedron, and the two component tetrahedra are swapped by the cube's central inversion
(x → −x). Each is its own mirror image — a regular tetrahedron is achiral — so the pair is
not a pair of chiralities.
...
1. **The tetrahedral angle is the cube's alternating-vertex angle**, and the
   ...
   This gives a second route to 54.74°: the direction cosine of the body diagonal with
   any axis is 1/√3, so its angle to an edge is arccos(1/√3). The two routes — half the
   tetrahedral angle, and the cube-diagonal angle — agree because they are one fact seen
   from two sides: the tetrahedron is half the cube.
...
- **It is parameter-free.** The identity cos θ = −1/(*N*−1) has no adjustable
  ...
  tuned.
- **It is consistent from both sides.** The value 54.74° is reached from the tetrahedron
  and from the cube (§4). These are one fact seen two ways, not independent evidence —
  but their agreement is a check on the computation.
...
nonzero in 2012 [Daya Bay; RENO], and the measured sin²θ₁₂ is about 0.31, not 1/3. The
tetrahedral 1/3 is thus at most a leading approximation that the measurements have
since departed from; it is recorded here as an illustration, not as support.
...
is reached consistently from the tetrahedron and from the cube. The same geometry is repeatedly
