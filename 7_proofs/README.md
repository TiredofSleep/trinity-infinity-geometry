...
Every load-bearing claim in this framework has a verification script in [`../_verification_scripts/`](../_verification_scripts/) that runs in seconds on a stock Python install. Total runtime to verify the full load-bearing stack: **under one minute**.
...
python ../_verification_scripts/meta_extension.py
```
...
```bash
PYTHONIOENCODING=utf-8 python ../_verification_scripts/<script>.py
```
