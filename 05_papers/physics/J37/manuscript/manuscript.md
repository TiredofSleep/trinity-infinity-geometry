...
J37's headline numerical claims are verified by two short Python scripts (numpy only) that run in $< 5$ s on a standard laptop. The script index, in order:
...
**Before J37:** the connection between TIG and SO(10) GUT was "TIG's so(10) and SO(10) GUT's so(10) are abstractly isomorphic" — trivially true, since there is only one $\mathfrak{so}(10)$ up to iso.

**After J37:** the connection is, at the structural level:
...
J29 (so(10)) TSML+BHML jointly close at so(10) = D_5 at dim 45
   |
   ▼
J37          Cl(0, 10) lift: 100 anticommutation rels, 32 = 16+16 chirality,
...
Each level is machine-verified at $\le 10^{-15}$ residuals. Each level is honestly scoped: J29 is a structural identification via Cartan classification; J37 is an alignment hypothesis test (the Cl(0,10) construction + the chirality-flip identity); J24 develops the two-readings framing.
...
* B.R. Sanders, M. Gish. *The CL Forcing Axioms: A1–A9 Uniquely Force the Canonical Composition Lattice on $\mathbb{Z}/10\mathbb{Z}$ Preserving a Designated 4-Core*, submitted to *Algebraic Combinatorics*, 2026 (J17).
...
* B.R. Sanders, M. Gish. *TSML 73 Cells / BHML 28 Cells: Lens-Invariant Cell Counts on the Z/10Z Composition Lattice*, submitted to *Experimental Mathematics*, 2026 (J32).
