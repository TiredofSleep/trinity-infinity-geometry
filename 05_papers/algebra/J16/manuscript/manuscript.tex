% J16 — The CL_TSML Composition Lattice on Z/10Z:
...
\item \textbf{Independence (with partial witnesses)}
(Theorem~\ref{thm:independence}): for each
$i \in \{1, \ldots, 7\}$, we exhibit an explicit witness magma
$M_{i}$. The witnesses $M_{2}$ and $M_{7}$ are \emph{strict}: they
satisfy all axioms in $\{S_{j} : j \neq i\}$ and fail only $S_{i}$,
establishing that $S_{2}$ and $S_{7}$ are not logical consequences
of the remaining six axioms. The witnesses $M_{1}, M_{3}, M_{4},
M_{5}, M_{6}$ are \emph{partial}: each fails $S_{i}$ together with
one or two additional axioms (see §\ref{sec:independence}
Remark~\ref{rem:partial-witnesses}); these demonstrate that no
single $S_{i}$ is derivable from the subset of axioms satisfied by
the corresponding partial witness. Construction of strict
independence witnesses for the remaining five axioms is an open
problem.
\end{enumerate}
...
their values. The remaining $56 - 10 = 46$ cells are non-exceptional
by $S_{6}$ (i.e., have value in $\{0, 7\}$), and we now show by a
count-elimination argument that they must all equal $7$.

By $S_{5}$ the total VOID count is exactly $17$, and Step~$1$ has
already placed $17$ VOID cells in row $0$ and column $0$
(off-column-$7$ and off-row-$\{0, 7\}$ respectively). The total VOID
count is therefore already saturated at $17$ before Step~$4$ begins;
hence the $46$ remaining cells cannot be VOID. Since they are not
exceptional (by $S_{6}$) and not VOID (by the count-saturation of
$S_{5}$ just established), they must be HARMONY $= 7$. This forces
$46$ HARMONY values in a single step.

We verify the overall count against $S_{5}$: HARMONY count after
Step~$4$ is $19 + 8 + 46 = 73$, VOID count is $17$, exceptional
count is $10$. These match $S_{5}$ exactly.
...
%==============================================================
\section{Independence and partial witnesses}
\label{sec:independence}
%==============================================================

We now address the non-redundancy of the axiom system. The
witnesses $M_{2}$ and $M_{7}$ exhibited below are \emph{strict}
independence witnesses (each satisfies all six other axioms while
failing the targeted one); the witnesses $M_{1}, M_{3}, M_{4},
M_{5}, M_{6}$ are \emph{partial} witnesses, each failing the
targeted axiom together with one or two additional axioms
(see Remark~\ref{rem:partial-witnesses}). The interaction of the
count constraint $S_{5}$, the position constraint $S_{6}$, and the
puncture constraint $S_{2}$ on the $\Z/10\Z$ substrate is
combinatorially tight; construction of strict witnesses for the
remaining five axioms is left open.
...
(\texttt{cl\_forcing.py}) constructs and checks one such $M_{1}$
(as a partial witness, failing $S_{1}$ together with $S_{5}$ and
$S_{7}$).
...
$17$, exceptional $10$ (preserved by the swap). The verification
script \texttt{cl\_forcing.py} constructs $M_{3}$ as a partial
witness failing $\{S_{2}, S_{3}\}$.
...
cells, breaking $S_{5}$ in count alongside the propagated
$\{S_{5}, S_{6}\}$ failure noted in Remark~\ref{rem:partial-witnesses}.
The verification script \texttt{cl\_forcing.py} constructs an explicit
partial $M_{5}$.
...
in the reference script \texttt{cl\_forcing.py}. Each $M_{i}$
is a $10 \times 10$ matrix; the script checks, for each $i$, the
axiom-status vector $(S_{1}, \ldots, S_{7})$ of $M_{i}$ and reports
the fail-set. The strict witnesses $M_{2}$ and $M_{7}$ each fail
exactly one axiom (the targeted $S_{i}$); the partial witnesses
$M_{1}, M_{3}, M_{4}, M_{5}, M_{6}$ each fail $S_{i}$ together with
one or two additional axioms, as tabulated in
Remark~\ref{rem:partial-witnesses}. Construction of strict witnesses
for the five partial cases is an open problem.
\end{proof}

\begin{remark}[On the constructions]
\label{rem:partial-witnesses}
...
\smallskip
\noindent\textbf{Honest scope of the independence claim.} The
witness magmas $M_{2}$ and $M_{7}$ are \emph{strict} independence
witnesses (each fails only the targeted axiom, satisfying all six
others). The witnesses $M_{1}, M_{3}, M_{4}, M_{5}, M_{6}$ are
\emph{partial} witnesses: each fails the targeted axiom $S_{i}$
together with one or two additional axioms (see the per-witness
fail-set tabulation immediately above). Hence Theorem~\ref{thm:independence}
establishes (a) that $S_{2}$ and $S_{7}$ are not derivable from the
remaining six axioms (strict independence of these two), and
(b) that no single $S_{i}$ for $i \in \{1, 3, 4, 5, 6\}$ is
derivable from the specific subsets of axioms satisfied by the
corresponding partial witnesses. Construction of strict
independence witnesses for the remaining five axioms (i.e., magmas
that fail \emph{only} $S_{i}$ for $i \in \{1, 3, 4, 5, 6\}$) is an
open problem, reflecting the tight combinatorial interaction of
$S_{2}$, $S_{5}$, and $S_{6}$ on the $\Z/10\Z$ substrate.
\end{remark}
...
magma $M_{i}$ explicitly and reports the full axiom-status vector
$(S_{1}, \ldots, S_{7})$ of $M_{i}$. The witnesses $M_{2}$ and
$M_{7}$ are strict independence witnesses (failing only the targeted
axiom); the witnesses $M_{1}, M_{3}, M_{4}, M_{5}, M_{6}$ are
partial witnesses, each failing the targeted axiom together with
one or two additional axioms (see §\ref{sec:independence}
Remark~\ref{rem:partial-witnesses}). Construction of strict
witnesses for the remaining five axioms is an open problem.
\end{enumerate}
