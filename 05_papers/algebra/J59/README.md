# J59 — Algebraic Rigidity of the σ-Magma on ℤ/10ℤ: Simplicity, Trivial Automorphism Group, and Unique Sub-Magma
...
**Tier:** A (all results PROVED, all computations exhaustive at machine precision)
...
- **J35** (Four-Core Fusion-Closure) establishes that the 4-core $\mathcal{C} = \{0, 7, 8, 9\}$ is jointly closed under TSML, BHML, and CL_STD. NEGATIVE finding in this paper (§3): $\mathcal{C}$ is NOT closed under the σ-magma, so J35's 4-core does not transfer to the σ-magma's algebraic structure.
...
- **J58** is the companion mod-3 paper on the Lo Shu D₄ orbit; the two papers are independent but share an "extremality through exhaustive verification" methodology.
