...
This paper is a companion to [J24] (cosmological dark-energy realization of the same logarithmic potential, $V(\Xi) = \Lambda^4 \Xi \log \Xi$, freezing-quintessence regime), to [J32] (Crossing Lemma), and to [J48] (companion paper applying the BB bridge to Yang-Mills mass gap).
...
Fix $N$ squarefree with smallest prime factor $p_1$. The discrete composition algebra of interest is the pair of binary operations $T, B : \mathbb{Z}/N\mathbb{Z} \times \mathbb{Z}/N\mathbb{Z} \to \mathbb{Z}/N\mathbb{Z}$ studied in [J14, J15]; the relevant invariant is the non-associativity defect

$$\sigma(N) = \sum_{x,y,z} \mathbb{1}\!\left[T(T(x,y),z) \neq T(x, T(y,z))\right] / N^3,$$

with the analogous definition for $B$. The companion paper [J14] proves a $\sigma(N) \leq 2/N$ rate theorem on a specific squarefree-modular family, hence $\sigma(N) \to 0$ as $N \to \infty$.

### 3.2 The Crossing Lemma reading

The Crossing Lemma [J32] identifies "information generation" with the failure of partition separability: an evolution generates information only when it acts non-trivially on the quotient $\mathbb{Z}/N\mathbb{Z} \to \prod_i \mathbb{Z}/p_i\mathbb{Z}$ given by the Chinese Remainder Theorem. A separability-preserving evolution leaves the CRT factorization intact and cannot be a source of new information.
...
This is the same potential studied cosmologically in [J24] as freezing/thawing quintessence; the connection is structural (logarithmic potential forced by separability), not derivational.
...
This paper carries no TSML / BHML lens dependence. The mathematical content is real-analysis + nonlinear PDE; the discrete side cites [J14, J15, J32] but does not condition on the lens taxonomy.
...
- [J14] Sanders, B.R., Gish, M. (2026). "Non-Associativity Decay in Binary Composition Tables over $\mathbb{Z}/N\mathbb{Z}$." Submitted to *JCT-A*.
- [J15] Sanders, B.R., Gish, M. (2026). "Joint Closure, Per-Coordinate Fuse Data, and a Closed-Form Algebraic Attractor of Two Commutative Binary Operations on $\mathbb{Z}/10\mathbb{Z}$." Submitted to *Algebraic Combinatorics*.
- [J24] Sanders, B.R., Gish, M., Johnson, H.J. (2026). "Freeze-Thaw Transit: Dual-Regime Scalar Dark Energy with Analytic Vacuum at $e^{-1}$ from a Logarithmic Potential." Submitted to *JCAP*.
- [J32] Sanders, B.R., Mayes, B. (2026). "Crossing Lemma." Submitted to *JCT-A* / *JPAA*.
- [J48] Sanders, B.R., Johnson, H.J. (2026). "The Yang-Mills Mass Gap Bridge." Companion JMP submission.
