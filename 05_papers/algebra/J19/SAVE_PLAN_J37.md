...
> *The matrix $T$ studied below arises in a discrete dynamical system on $\mathbb{Z}/10\mathbb{Z}$ developed in a separate research program (here named "TIG" only for citation purposes; details and motivation are reported in [Sanders & Mayes, J09, J11]). The present note treats $T$ as a fixed 10×10 integer matrix and studies its characteristic polynomial. No physical interpretation is claimed.*
...
§3 *Connection to the doubly-invariant subalgebra.* The exponent 16 in the factorization $\mathrm{disc}(g) = 2^{16} \cdot \ldots$ matches $\dim(\mathfrak{g}_0)$ where $\mathfrak{g}_0 \subset \mathfrak{so}(10)$ is the 16-dimensional doubly-invariant subalgebra studied in [J09 / J11]; the exponent 7 in $7^7$ matches the recurring entry 7 in $T$. We record this co-occurrence as a structural observation about the eigenvalue separations (the discriminant) versus the elementary symmetric functions (the coefficients). Whether this co-occurrence is structural or coincidental is open. No physical interpretation is offered here.
...
J19 was identified in its own README as the 3rd PRD paper of the quarter. Retargeting to LAA removes this entirely from PRD; the cap concern dissolves. LAA: this is a fresh venue for the J-series. Cap not in play.

J38's recommendation (folding into J44 — see SAVE_PLAN_J38) further reduces PRD load this quarter, which is independently good news for J44 / J44.
...
- [ ] Update J-series ordering doc (target-venue field for J19; per-venue cap on PRD recomputed)
