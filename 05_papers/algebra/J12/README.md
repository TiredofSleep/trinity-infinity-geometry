...
**WP source:** WP105 (closed-form attractor) + WP113 (PSLQ uniqueness); Galois content of J01's Theorem D extracted into its own self-contained treatment
...
The Galois-theoretic content of the four-core attractor — extracted from J01 (the *Journal of Algebra* corpus centerpiece, where it appears as Theorem D / Theorem 5.2 inside a six-check fusion-closure bundle) — and rebuilt as a self-contained 12-page algebraic-number-theory submission for *Communications in Algebra*. J12 differentiates from J01 by depth on the single Galois question: explicit irreducibility argument over `Q` (case-by-case + mod-7 cross-check), explicit cubic resolvent computation with rational root and quadratic-factor discriminant `-71`, explicit C_4-vs-D_4 distinction via irreducibility over `Q(sqrt(-71))`, explicit `Q(sqrt(3))`-factorization, and explicit Tschirnhaus reduction to LMFDB's canonical defining polynomial `x^4 - 7x^2 - 12x - 8`. J01 carries the broader fusion-closure picture; J12 carries the proof that justifies J01's bare assertion `Gal(f/Q) = D_4`.
...
- **J15** — *Joint Closure, Per-Coordinate Fuse Data, and a Closed-Form Algebraic Attractor of Two Commutative Binary Operations on Z/10Z.* Submitted to *Algebraic Combinatorics*. Provides the substrate (`(T, B)` tables and joint fuse data) that defines `F_{1/2}` and the four-core simplex.
- **J01** — *Joint Closure, a Universal Attractor, and an Algebraic Mixing Point for a Pair of Binary Operations on Z/10Z.* Submitted to *Journal of Algebra*. The corpus-centerpiece companion: J12 is the deeper standalone proof of J01's Theorem D Galois content.
...
- LaTeX header: `% J12 …` (was stale `% J22`); duplicate `\author{}` block split into separate `\author{Sanders} / \author{Gish}`
...
- [x] Dependencies → J15 and J01 cited as "submitted to [venue]"
