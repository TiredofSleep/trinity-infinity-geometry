...
# We use a negative lookahead/lookbehind for word chars to avoid partial matches
# but special-handle the longer keys (which contain underscores) properly.
# Build alternation:
PAT = re.compile(
    r"(?<![A-Za-z0-9_])("
...
SKIP_FILENAMES = {
    # Don't auto-edit historical session-notes; they're a journal of past states.
    # If user wants those updated they can run a follow-up.
    "MEMORY.md",
    "memory.md",
}
