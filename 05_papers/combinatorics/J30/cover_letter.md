...
The primary J30-specific verification script is `manuscript/verification/verify_J30.py` (Python standard library only, no external dependencies; runtime << 1 second; **9/9 PASS**). It verifies, by direct enumeration on the explicit 10 × 10 CL_TSML and CL_BHML tables: (i) (Z/10Z)* closed under × mod 10; (ii) cyclic group of order 4 generated by g = 3; (iii) CL_TSML | C×C with image {3, 7} and 14/2 distribution at PROGRESS cells (3, 9), (9, 3); (iv) CL_BHML | C×C with image {0, 2, 4, 6, 8} disjoint from C and witness CL_BHML[1][1] = 2 (Proposition 4.1); (v) size-4 enumeration giving 78 TSML-closed (all containing HARMONY = 7), 1 BHML-closed, 1 jointly closed = {0, 7, 8, 9} (Theorem 4.3); (vi) the contrast C ∩ joint = {7, 9}; (vii) generator-selection arithmetic 3³ ≡ 7 vs 7³ ≡ 3 (mod 10) with primitive-roots set = {3, 7} (Theorem 5.1); (viii)–(ix) the CREATION and DISSOLUTION orbits under × 3. A companion script `manuscript/verification/4core_verification.py` (6 checks supporting the joint-4-core context of Theorem 4.3) is retained as cross-reference to Sanders–Gish J15/J01; it is not the primary verification for J30.
