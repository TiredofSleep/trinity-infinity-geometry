...
| **J03** | The First-G Event and a Discrete $\sinc^2$ Identity | *Integers — Electronic Journal of Combinatorial Number Theory* | 2026-05-13 |
...
J03 lands as the third number_theory paper (Fork A initial port 2026-05-13; **substance upgrade 2026-05-13** with the spectral-characterization theorem). Authors: Sanders + Gish. Two verification scripts PASS at machine precision: `verify_J03.py` **7/7 PASS** (closed form, synchronization, continuum limit, endpoint identity, **Theorem 5.2 obstruction-zero correspondence**, **Cor 5.4 asymptotic zero density**; max deviation 4.44 × 10⁻¹⁶ for the closed form; 900/900 cell-level matches for the boolean equivalence $f_b(k)=0 \Leftrightarrow \gcd(k,b)>1$); `proof_first_g_event.py` zero counterexamples across 22,367 (b, k) pairs for 305 squarefree b ∈ [2, 500] (runtime < 3 s). The substantive contribution is **Theorem 5.2**: for every b > 1 with distinct prime factors $p_1, \dots, p_r$, the spectral product $f_b(k) := \prod_{j=1}^r R(k, p_j)$ of discrete Fejér quotients vanishes at integer $k \ge 1$ iff $\gcd(k, b) > 1$; the zero set of $f_b$ in $\mathbb{N}$ equals $\bigcup_j p_j\mathbb{N}$, giving a single, naturally-defined product spectral function that acts as a continuous-in-$k$ indicator for the obstruction event. The synchronization theorem (5.1) is the special case at the smallest spectral zero; the asymptotic density theorem (Cor 5.4) gives the zero density as the Euler product $1 - \varphi(\mathrm{rad}(b))/\mathrm{rad}(b)$. Earlier classical theorems retained for completeness: 3.1 (First-G localization, one-line gcd proof), 4.2 (closed-form Fejér identity), 6.1 (continuum limit). §1 "Positioning and contribution" replaces the previous "what is new is the packaging" admission with explicit framing of the spectral characterization as the contribution. The *Integers* per-quarter cap was already exactly used by J04; J03 may need to defer or move to *AMM Notes* / *Math Magazine* if the cap is enforced strictly, but the substance upgrade strengthens the case for *Integers* directly.
