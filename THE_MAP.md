...
Six research programs that rarely speak to each other turn out to be six sides of one object. It is a **dictionary** — the same content written two ways:
...
The **center — where all three meet — is the shared alphabet**: `SL(2,ℤ)`, the primes, and `ζ`. This is why "pre-physics" is the right word: the center is pure mathematics, medium-independent. **Physics is what happens when you spell with this alphabet in a medium** — quantize `xp` and you get Berry–Keating; put it in a fluid and you get Migdal; in a gauge field, Langlands; in a 3-manifold, arithmetic topology. Same letters, different ink.
...
- **Étienne Ghys · Vladimir Arnold** — *dynamics whose orbits are number theory.* Ghys proved the periodic orbits of the modular/Lorenz flow **are** modular knots (continued-fraction / `SL(2,ℤ)` objects); Arnold's tongues are the mode-locking → Farey structure. [Knots and dynamics (ICM 2006)](https://perso.ens-lyon.fr/ghys/articles/knotsdynamics.pdf). *Rigorous.*
...
- **Berry & Keating · Montgomery–Odlyzko–Dyson · Sarnak** — *the spectrum of matter ↔ the zeros.* `H = xp` conjectures the zeros as a quantum spectrum; the zeros' spacing is **GUE random-matrix** statistics — the same law as heavy-nucleus spectra. [H=xp revisited (PRL 2011)](https://link.aps.org/doi/10.1103/PhysRevLett.106.200201). *Frontier-but-serious; RMT link rigorous.*
...
- **Barry Mazur · Masanori Morishita · Yuri Manin** — *primes as relational space.* Arithmetic topology: primes ↔ knots, number fields ↔ 3-manifolds (the MKR dictionary). [Morishita, *Knots and Primes*](https://link.springer.com/book/10.1007/978-1-4471-2158-9). *Rigorous.*
...
- The CL composition tables (`TSML`, `BHML`) are **commutative non-associative magmas**. The **non-associative fraction** measures information capacity: where composition is *path-dependent*, meaning lives; where it is associative, the cell is a **dead zone** (path-independent, carrying nothing). Measured: `TSML ≈ 12.6%` non-associative, `BHML ≈ 49.8%`.
...
- **`F₇` is a spine, not the skeleton.** The Farey sequence of order 7 recurs in this project's constants (`HARMONY = 7`, `T* = 5/7`, `VOID = 17 = |F₇ ∩ (0,1)|`, `389 = 10² + 17²` from two `F₇` counts) — a real, repeated presence — but no single generating map makes `σ` and the CL tables `F₇`-exclusive.
